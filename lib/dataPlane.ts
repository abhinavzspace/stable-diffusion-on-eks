...
import {
  EbsThroughputTunerAddOn,
  EbsThroughputTunerAddOnProps,
} from "./addons/ebsThroughputTuner";
...
import {
  SharedComponentAddOn,
  SharedComponentAddOnProps,
} from "./addons/sharedComponent";
...
export default class DataPlaneStack {
  ...
  constructor(
    ...
    const s3SyncEFSAddOnParams: S3SyncEFSAddOnProps = {
      ...
      efsFilesystem: blueprints.getNamedResource(
        "efs-model-storage"
      ) as efs.IFileSystem,
    };
    ...
    dataplaneProps.modelsRuntime.forEach((val, idx, array) => {
      const sdRuntimeParams: SDRuntimeAddOnProps = {
        ...
        outputBucket: blueprints.getNamedResource(
          "outputS3Bucket"
        ) as s3.IBucket,
    ...
    const blueprint = blueprints.EksBlueprint.builder()
      ...
      .resourceProvider(
        "inputSNSTopic",
        new SNSResourceProvider("sdNotificationLambda")
      )
      .resourceProvider(
        "outputSNSTopic",
        new SNSResourceProvider("sdNotificationOutput")
      )
      .resourceProvider(
        "outputS3Bucket",
      ...
      .resourceProvider(
        "efs-model-storage",
        new blueprints.CreateEfsFileSystemProvider(efsParams)
      )
      .clusterProvider(new blueprints.MngClusterProvider(MngProps))
      .build(scope, id + "Stack");

    // Workaround for permission denied when creating cluster
    const handler = blueprint.node
      .tryFindChild("@aws-cdk--aws-eks.KubectlProvider")!
    ...
    const clusterNameCfnOutput = cluster.node.findChild(
      "ClusterName"
    ) as cdk.CfnOutput;
    clusterNameCfnOutput.overrideLogicalId("ClusterName");

    const configCommandCfnOutput = cluster.node.findChild(
      "ConfigCommand"
    ) as cdk.CfnOutput;
    configCommandCfnOutput.overrideLogicalId("ConfigCommand");

    const getTokenCommandCfnOutput = cluster.node.findChild(
      "GetTokenCommand"
    ) as cdk.CfnOutput;
